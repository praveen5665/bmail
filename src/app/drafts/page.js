...
export default function Drafts() {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      if (result.success && result.emails.length > 0) {
        ...
        // Filter emails where the current user is the sender and email is a draft
        const draftEmails = result.emails.filter(email => 
          email.from && 
          email.from.toLowerCase() === user.ethAddress.toLowerCase() &&
          email.isDraft
        );
        console.log('Filtered draft emails:', draftEmails);
  ...
  return (
    <EmailLayout title="Drafts" emailCount={filteredEmails.length} activeTab="drafts">
      {/* Search and Filter */}
      ...
      <div className="divide-y divide-gray-200">
        {loadingEmails ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">Start composing a new email to create a draft.</p>
            <div className="mt-6">
              <button
            ...
            {filteredEmails.map((email, index) => (
              ...
              >
                <div 
                  ...
                  onClick={() => router.push(`/compose?id=${email.id}`)}
                >
