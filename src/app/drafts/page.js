...
export default function Drafts() {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Format emails for display - only show drafts
        const formattedEmails = result.emails
          .filter(email => email.sender === user.ethAddress && email.isDraft) // Only show drafts
          .map(email => {
  ...
  const continueDraft = (emailId) => {
    router.push(`/compose?draft=${emailId}`);
  };
        ...
        <div className="flex space-x-6">
          ...
          <div className="w-64 bg-white rounded-lg shadow-sm p-4">
            ...
            <nav className="space-y-1">
              ...
              <a href="/drafts" className="block px-3 py-2 bg-blue-50 text-blue-700 rounded-md font-medium">
                Drafts
              </a>
              ...
              <ul className="divide-y divide-gray-200">
                {emails.map((email) => (
                  <li key={email.id} className="hover:bg-gray-50 transition-colors">
                    <button 
                      onClick={() => continueDraft(email.id)}
                      className="block px-6 py-4 w-full text-left"
                    >
